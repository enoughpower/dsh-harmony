...
- 任务监听（实况窗）：动态轮询 session.list（有任务 5s / 无任务 15s）作为统一数据源；任务开始→实况窗发布（蓝鲸 logo + 品牌蓝胶囊）、进度→实时更新、完成→收起；首页/小组件同源（快照 + 实时订阅 + 主动 updateForm）；后台保活（DATA_TRANSFER + 常驻通知「DSH 任务监听中」）；设置「任务监听」开关可关闭（关闭后仅保留主动刷新）

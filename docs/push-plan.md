...
> 状态：**方案推进中**（2026-08-24；临时异常测试渠道已实测打通，华为 AGC 待自分类权益审核）
...
>
> ### ✅ 临时异常测试渠道（已验证接入，非对外功能）
> - 目的：自有推送（华为 AGC，自分类权益审核中）通过前，验证「任务进度 → 手机推送」链路；代码保留在 `push-notify.js`（`SENDER=test`），便于异常排查
> - 接入：发送端双通道 `SENDER` 切换；修复 dsh-pocket 登录（表单字段 token）
> - 实测：仅三场景推送策略（会话结束汇报 / 异常超时 / 中途收集信息）均推送成功（200；发送端限流 3 秒 1 条，已加 3.1s 串行节流）
> - **替换**：自有推送（华为 getToken）通过后 → .env 切 `SENDER=huawei` → 测试渠道相关保留在分支历史
